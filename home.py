...
def getPage1():
    ...
    basicPageSetup(1)
    heading.configure(text="Register Criminal and lost People")
    right_frame.configure(text="Enter Details")

    btn_grid = tk.Frame(left_frame, bg="#1C1C1C")
    btn_grid.pack()

    tk.Button(btn_grid, text="Select Images", command=lambda: selectMultiImage(opt_menu, menu_var), font="Arial 15 bold", bg="#E31C25",
    ...
    for i, field in enumerate(input_fields):
        ...
        label = tk.Text(row, width=20, height=1, bg="#1C1C1C", fg="#ffffff", font="Arial 13", highlightthickness=0, bd=0)
        ...
        if(i != ip_len-1):
            ...
            opt_menu.configure(font="Arial 13", bg="#E31C25", fg="white", bd=0, highlightthickness=0, activebackground="#90ceff")
...
def startRecognition():
    ...
    if (len(face_coords) == 0):
        ...
        (frame, recognized, recog_names) = recognize_face(model, frame, gray_frame, face_coords, names)
...
def videoLoop(model, names):
    ...
    try:
        while not thread_event.is_set():
            ...
            # Detect Faces
            face_coords = detect_faces(gray_frame)
            (frame, recognized, recog_num) = recognize_face(model, frame, gray_frame, face_coords, names)
            if len(recognized):
                print(recognized)
                for num in recog_num:
                    marked=[0]*len(names)
                    if marked[num] ==0:
                        marked[num] = 1
                        sendMsg(names[num]) 
                        print(names[num]+ "has been spotted ")
...
def getPage4():
    ...
    basicPageSetup(3)
    heading.configure(text="Vedio Footage")
    right_frame.configure(text="Detected Criminals")
    left_frame.configure(pady=40)
    btn_grid = tk.Frame(left_frame, bg="#1C1C1C")
    btn_grid.pack()
...
######################################## Home Page ####################################
tk.Label(pages[0], text="Criminal Identification System", fg="white", bg="#1C1C1C",
      font="Arial 35 bold", pady=30).pack()

logo = tk.PhotoImage(file = "logo.png")
tk.Label(pages[0], image=logo, bg="#1C1C1C").pack()

btn_frame = tk.Frame(pages[0], bg="#1C1C1C", pady=20)
btn_frame.pack()
...
for btn in btn_frame.winfo_children():
    btn.configure(font="Arial 20", width=17, bg="#283e3c", fg="white",
